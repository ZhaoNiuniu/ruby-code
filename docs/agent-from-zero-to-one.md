...
- 当前阶段：命令行可用的 OpenAI-compatible agent，具备进程内 conversation session、原生 tool calling、todo planning、subagent 上下文隔离、按需 skill 加载、本地五工具、hook registry 和执行前权限闸门。
- 当前重点：把会话连续性、任务规划、上下文隔离、按需上下文加载、工具执行、安全审批、hook 扩展和 provider 扩展沉淀成稳定工程边界。
...
| 2026-07-01 | `load_skill` 只读取项目内 `skills/` 启动快照，并通过 `SkillCatalogHook` 注入短目录 | 用按需加载降低 system prompt token，保持 workspace-only 边界，避免把个人/插件技能混入项目 agent |
...
- `TodoReminderHook` 在注册了 `todo_write` 时，通过 `USER_PROMPT_SUBMIT` 给复杂/改动类任务注入规划提醒。
- `SkillCatalogHook` 在存在非空 skill catalog 时，通过 `USER_PROMPT_SUBMIT` 注入短目录和 `load_skill` 使用规则，并排在 `TodoReminderHook` 之前。
...
- 只有注册了 `todo_write` 时，默认 hook 才注入 reminder，避免提示不可用工具。
...
- 子 agent system prompt 使用 `SUB_SYSTEM + 父 systemPrompt`。
- `DefaultAgent` 通过保留 metadata key `agent.systemPrompt` 把有效父 prompt 传给工具，并覆盖用户同名 metadata。
- 子 agent 工具集只包含 `todo_write/read_file/glob/bash`。
...
- `SkillCatalogHook` 注入 `name + description` 短目录。
...
- `agent-core`: loop、session、max steps、tool call id、工具失败回传、hook registry、权限闸门、registry duplicate、subagent 上下文隔离。
